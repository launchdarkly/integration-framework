...
aws s3 cp manifest.schema.json s3://$BUCKET_NAME/integrations/$REV
aws s3 cp latest s3://$BUCKET_NAME/latest.json --acl public-read --metadata-directive REPLACE --content-type "application/json"
